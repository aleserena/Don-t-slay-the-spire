...
describe('Card Interaction Improvements', () => {
  ...
  describe('Unified Card Selection Behavior', () => {
    it('should handle targeting cards with enhanced visual feedback', () => {
      ...
      if (targetingCard) {
        ...
        expect(['strike', 'bash', 'twin_strike', 'body_slam', 'anger'].some(id => 
          targetingCard.baseId === id
        )).toBe(true);
      } else {
    ...
    it('should handle confirmation cards with enhanced visual feedback', () => {
      ...
      const confirmationCard = store.hand.find(card => 
      ...
      if (confirmationCard) {
        ...
        expect(['defend', 'inflame', 'metallicize', 'demon_form'].some(id => 
          confirmationCard.baseId === id
        )).toBe(true);
      } else {
        // If no confirmation card in hand, verify the classification system works
        const allCards = getAllCards();
        const confirmationCards = allCards.filter(card => 
          !cardNeedsTarget(card) && 
          card.baseId !== 'cleave' && 
          card.baseId !== 'whirlwind'
        );
    ...
    it('should clear targeting state when clicking confirmation cards', () => {
      ...
      if (targetingCard && confirmationCard) {
        ...
        const targetingCost = typeof targetingCard.cost === 'number' ? targetingCard.cost : 0;
        const confirmationCost = typeof confirmationCard.cost === 'number' ? confirmationCard.cost : 0;
    ...
    it('should handle multi-target cards consistently', () => {
      ...
      const whirlwindCard = store.hand.find(card => card.baseId === 'whirlwind');
      ...
      if (multiTargetCard) {
        ...
        expect(store.player.energy).toBeGreaterThanOrEqual(
          multiTargetCard.baseId === 'whirlwind' ? 0 : 
          typeof multiTargetCard.cost === 'number' ? multiTargetCard.cost : 0
        );
      }
    });
  });
  ...
  describe('Card State Management', () => {
    it('should properly manage card selection states', () => {
      ...
      const targetingCards = allCards.filter(card => cardNeedsTarget(card));
      const nonTargetingCards = allCards.filter(card => !cardNeedsTarget(card));
      ...
      targetingCards.forEach(card => {
        expect(['attack'].includes(card.type) || 
               ['body_slam'].includes(card.baseId)).toBe(true);
      });
