...
describe("Rest Site Upgrade Functionality", () => {
  ...
  describe("Rest Site State Management", () => {
    it("should have card upgrade modal state management", () => {
      const { openCardUpgradeModal, closeCardUpgradeModal } =
  ...
  describe("Card Upgrade Action", () => {
    ...
    it("should upgrade a specific card and return to map", () => {
      ...
      const upgradableCard = useGameStore
        .getState()
        .drawPile.find((card) => !card.upgraded);
    ...
    it("should upgrade cards in discard pile", () => {
      ...
      const upgradedCard = newState.discardPile.find(
        (card) => card.id === cardId,
      );
      expect(upgradedCard).toBeDefined();
      expect(upgradedCard!.upgraded).toBe(true);
    ...
    it("should not upgrade already upgraded cards", () => {
      ...
      // First upgrade
      upgradeCardAtRest(cardId);
      const firstUpgrade = useGameStore
        .getState()
        .drawPile.find((c) => c.id === cardId);
      ...
      upgradeCardAtRest(cardId);
      const afterSecondUpgrade = useGameStore
        .getState()
        .drawPile.find((c) => c.id === cardId);
  ...
  describe("Card Upgrade Integration", () => {
    it("should properly upgrade Strike card", () => {
      ...
      // Find a Strike card
      const strikeCard = useGameStore
        .getState()
        .drawPile.find((card) => card.baseId === "strike");
      expect(strikeCard).toBeDefined();

      upgradeCardAtRest(strikeCard!.id);

      const upgradedStrike = useGameStore
        .getState()
        .drawPile.find((card) => card.id === strikeCard!.id);
    ...
    it("should properly upgrade Defend card", () => {
      ...
      // Find a Defend card
      const defendCard = useGameStore
        .getState()
        .drawPile.find((card) => card.baseId === "defend");
      expect(defendCard).toBeDefined();

      upgradeCardAtRest(defendCard!.id);

      const upgradedDefend = useGameStore
        .getState()
        .drawPile.find((card) => card.id === defendCard!.id);
    ...
    it("should properly upgrade Bash card", () => {
      ...
      // Find a Bash card
      const bashCard = useGameStore
        .getState()
        .drawPile.find((card) => card.baseId === "bash");
      expect(bashCard).toBeDefined();

      upgradeCardAtRest(bashCard!.id);

      const upgradedBash = useGameStore
        .getState()
        .drawPile.find((card) => card.id === bashCard!.id);
      expect(upgradedBash!.name).toBe("Bash+");
      expect(upgradedBash!.description).toBe(
        "Deal 10 damage. Apply 2 Vulnerable.",
      );
      expect(upgradedBash!.upgraded).toBe(true);

      // Check that damage effect was updated
      const damageEffect = upgradedBash!.effects?.find(
  ...
  describe("Modal State Consistency", () => {
    ...
    it("should clear selected card when closing upgrade modal", () => {
      const { openCardUpgradeModal, closeCardUpgradeModal, setSelectedCard } =
        useGameStore.getState();
