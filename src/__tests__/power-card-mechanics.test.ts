...
describe("Power Card Mechanics", () => {
  ...
  describe("Power Card Attachment", () => {
    it("should attach power cards to player and discard them normally", () => {
      ...
      const inflameTemplate = allCards.find(
        (c: Card) => c.baseId === "inflame",
      );
      ...
      const initialPowerCardsCount =
        useGameStore.getState().player.powerCards.length;
      ...
      expect(
        finalState.discardPile.find((c) => c.id === "inflame_test"),
      ).toBeDefined();

      // Power card should be attached to player
      expect(finalState.player.powerCards.length).toBe(
        initialPowerCardsCount + 1,
      );

      // Player should have gained strength
      const strengthEffect = finalState.player.statusEffects.find(
        (e) => e.type === StatusType.STRENGTH,
      );
    ...
    it("should allow multiple copies of the same power card to be attached", () => {
      ...
      const finalState = useGameStore.getState();
      const discardedInflameCards = finalState.discardPile.filter(
        (c) => c.baseId === "inflame",
      );
    ...
    it("should stack effects when multiple copies of the same power card are played", () => {
      ...
      const availableNodes =
        gameState.map?.nodes.filter(
          (n) => n.available && n.type === "combat",
        ) || [];
      const combatNode = availableNodes[0];
      store.selectNode(combatNode.id);
      ...
      const initialStrength = initialState.player.statusEffects.find(
        (e) => e.type === StatusType.STRENGTH,
      );
      ...
      const strengthAfterFirst = afterFirst.player.statusEffects.find(
        (e) => e.type === StatusType.STRENGTH,
      );
      ...
      const strengthAfterSecond = afterSecond.player.statusEffects.find(
        (e) => e.type === StatusType.STRENGTH,
      );
  ...
  describe("Power Card Clearing", () => {
    ...
    it("should make power cards available again in future fights", () => {
      ...
      const drawnInflame = stateWithCard.hand.find(
        (c) => c.baseId === "inflame",
      );
      ...
      if (drawnInflame) {
        ...
        expect(
          afterPlay.discardPile.find((c) => c.baseId === "inflame"),
        ).toBeDefined();
        ...
        expect(
          afterCombat.drawPile.find((c) => c.baseId === "inflame"),
        ).toBeDefined();
      }
    });
  });
