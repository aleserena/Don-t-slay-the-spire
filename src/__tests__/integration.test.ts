...
describe("Integration Tests", () => {
  ...
  describe("Complete Game Flow", () => {
    ...
    it("should handle card playing and combat mechanics", () => {
      ...
      const attackCard = store.hand.find(
      ...
      if (attackCard) {
        ...
        const newStore = useGameStore.getState();
        const cardCost =
          typeof attackCard.cost === "number" ? attackCard.cost : 0;
        expect(newStore.player.energy).toBe(initialEnergy - cardCost);
        expect(
          newStore.hand.find((c) => c.id === attackCard.id),
        ).toBeUndefined();
      }
    });
    ...
    it("should handle map generation and progression", () => {
      ...
      const availableNodes = map.nodes.filter(
    ...
    it("should handle shop and progression systems", () => {
      ...
      const initialDeckSize =
        useGameStore.getState().drawPile.length +
        useGameStore.getState().discardPile.length;
    ...
    it("should maintain game state consistency", () => {
      ...
      const totalCards =
        store.hand.length +
        store.drawPile.length +
        store.discardPile.length +
        store.exhaustPile.length;
