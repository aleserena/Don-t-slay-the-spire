...
describe('Bug Fixes', () => {
  ...
  describe('Card Deck Preservation Bug Fix', () => {
    it('should preserve deck composition after completing a level', () => {
      ...
      const deckSizeAfterReward = storeAfterReward.drawPile.length + storeAfterReward.discardPile.length;
      ...
      const deckSizeAfterCombat = storeAfterCombat.drawPile.length + storeAfterCombat.discardPile.length + storeAfterCombat.hand.length;
    ...
    it('should preserve player gold and relics after starting combat', () => {
      ...
      expect(storeAfterCombat.player.energy).toBe(storeAfterCombat.player.maxEnergy);
    });
  });
  ...
  describe('Map Progression Bug Fix', () => {
    it('should only make next floor nodes available after completing a node', () => {
      ...
      if (startingNode) {
        ...
        const availableNodes = updatedMap.nodes.filter(node => node.available && !node.completed);
        ...
        const floor0AvailableNodes = updatedMap.nodes.filter(node => 
          node.y === 0 && node.available && !node.completed
        );
  ...
  describe('Game State Consistency', () => {
    ...
    it('should handle Anger card effect correctly', () => {
      ...
      const angerCards = afterPlay.discardPile.filter(card => card.name === 'Anger');
    ...
    it('should preserve deck through complete combat cycle: 10 cards -> use 5 -> win -> add 1 reward -> 11 cards total', () => {
      const { playCard, selectCardReward, startCombat } = useGameStore.getState();
      ...
      const attackCard = combatState.hand.find(card => card.damage && card.damage > 0);
      ...
      if (attackCard) {
        ...
        const totalCards = afterCombat.hand.length + afterCombat.drawPile.length + afterCombat.discardPile.length;
    ...
    it('should properly handle deck state transitions', () => {
      const { selectCardReward, skipCardReward, returnToMap } = useGameStore.getState();
  ...
  describe('Map Connectivity Fixes', () => {
    ...
    it('should not make same-floor nodes available after completing a node', () => {
      ...
      if (firstNode) {
        ...
        const sameFloorNodes = updatedMap.nodes.filter(n => 
          n.y === firstNode.y && n.id !== firstNode.id && n.available && !n.completed
        );
