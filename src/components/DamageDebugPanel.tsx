...
export const DamageDebugPanel: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 10px 0', color: '#ffd700' }}>🔍 Damage Debug Panel</h3>
      ...
      <div style={{ marginBottom: '15px' }}>
        ...
        {enemies.map((enemy) => (
          <div key={enemy.id} style={{ marginLeft: '10px', marginBottom: '8px' }}>
            <div><strong>{enemy.name}</strong></div>
            <div>Health: {enemy.health}/{enemy.maxHealth}</div>
            <div>Block: {enemy.block}</div>
            <div>Intent: {enemy.intent.type} {enemy.intent.value ? `(${enemy.intent.value})` : ''}</div>
