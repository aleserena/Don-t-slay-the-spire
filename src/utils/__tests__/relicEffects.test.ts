...
describe("Relic Effects", () => {
  ...
  describe("Energy Core", () => {
    it("should gain 1 energy at start of turn", () => {
      ...
      const result = processRelicEffects(
  ...
  describe("Philosopher's Stone", () => {
    it("should gain 1 energy at start of turn and give enemies strength at combat start", () => {
      ...
      const turnResult = processRelicEffects(
        RelicTrigger.TURN_START,
        mockPlayer,
        mockEnemies,
      );
      ...
      const combatResult = processRelicEffects(
        RelicTrigger.COMBAT_START,
        mockPlayer,
        mockEnemies,
      );
      expect(combatResult.enemies[0].statusEffects).toHaveLength(1);
      expect(combatResult.enemies[0].statusEffects[0].type).toBe(
        StatusType.STRENGTH,
      );
  ...
  describe("Bag of Marbles", () => {
    it("should apply vulnerable to all enemies at combat start", () => {
      ...
      expect(result.enemies[0].statusEffects[0].type).toBe(
      ...
      expect(result.enemies[1].statusEffects[0].type).toBe(
        StatusType.VULNERABLE,
      );
