...
describe("MapGeneration", () => {
  describe("generateMap", () => {
    ...
    it("should have proper node types distribution", () => {
      ...
      const typeCount = nodeTypes.reduce(
        (acc, type) => {
  ...
  describe("completeNode", () => {
    it("should mark node as completed and make connected nodes available", () => {
      ...
      if (startingNode) {
        ...
        startingNode.connections.forEach((connectionId) => {
          const connectedNode = updatedMap.nodes.find(
            (n) => n.id === connectionId,
          );
    ...
    it("should not modify map if node is not available", () => {
      const map = generateMap();
      const unavailableNode = map.nodes.find(
        (node) => node.y > 0 && !node.available,
      );
    ...
    it("should handle completing final floor nodes", () => {
      ...
      for (let floor = 0; floor < 14; floor++) {
        const availableNodes = currentMap.nodes.filter(
          (n) => n.y === floor && n.available,
        );
      ...
      const finalFloorNode = currentMap.nodes.find(
        (n) => n.y === 14 && n.available,
      );

      if (finalFloorNode) {
        const finalMap = completeNode(currentMap, finalFloorNode.id);
        const completedFinalNode = finalMap.nodes.find(
          (n) => n.id === finalFloorNode.id,
        );
