...
describe("Power Card Effects", () => {
  ...
  describe("Metallicize", () => {
    it("should gain 3 block at end of turn", () => {
      ...
      const result = processPowerCardEffects(
  ...
  describe("Multiple Power Cards", () => {
    it("should process multiple power cards in sequence", () => {
      ...
      const startResult = processPowerCardEffects(
        PowerTrigger.TURN_START,
        mockPlayer,
        mockEnemies,
      );
      ...
      expect(startResult.player.statusEffects[0].type).toBe(
        StatusType.STRENGTH,
      );

      // Test turn end (should only trigger Metallicize)
      const endResult = processPowerCardEffects(
        PowerTrigger.TURN_END,
        startResult.player,
        mockEnemies,
      );
