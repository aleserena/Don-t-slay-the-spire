      ...
      switch (effect.type) {
        case EffectType.DAMAGE:
          if (effect.target === TargetType.ENEMY) {
            ...
            const finalDamage = calculateDamage(effect.value, newEnemy, newPlayer);
        ...
        case EffectType.APPLY_STATUS:
          if (effect.statusType) {
            if (effect.target === TargetType.SELF) {
              ...
              newEnemy = applyStatusEffect(newEnemy, effect.statusType, effect.value) as Enemy;
        ...
        case EffectType.HEAL:
          if (effect.target === TargetType.SELF) {
            newEnemy.health = Math.min(newEnemy.maxHealth, newEnemy.health + effect.value);
          }
          break;
